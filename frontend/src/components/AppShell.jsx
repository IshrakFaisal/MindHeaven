...
const MOBILE_ITEMS = NAV_ITEMS.filter((item) => ['overview', 'mood', 'community', 'reports'].includes(item.id));

function MobileNavButton({ item, active, label, onClick }) {
  const { Icon } = item;
  return (
    <button
      type="button"
...
export default function AppShell({ activePage, onNavigate, user, onLogout, preferences, onOpenOnboarding, onLocaleChange, children }) {
  ...
  const labelFor = (item, short = false) => t(`nav.${item.id}`, short ? item.short : item.label);
  ...
  return (
    <div className={`wellness-shell relative min-h-screen overflow-hidden ${preferences?.compactMotion ? 'reduce-motion' : ''}`}>
      <a href="#main-content" className="skip-link">{t('shell.skip', 'Skip to main content')}</a>
        ...
        <div className="app-header-inner relative mx-auto flex h-20 items-center">
            ...
            {NAV_ITEMS.filter((item) => item.id !== 'profile').map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="app-nav-label">{labelFor(item)}</span>
                </button>
              );
            })}
          </nav>
          ...
          <div ref={accountRef} className="relative ml-auto hidden shrink-0 items-center gap-2 lg:flex">
            <button type="button" onClick={() => navigate('checkin')} className="primary-button min-h-11 rounded-full px-5 py-2.5">
              <Plus size={16} strokeWidth={3} /> {t('shell.checkIn', 'Check in')}
            </button>
            <button
              type="button"
            ...
            {accountOpen && (
              <div className="page-enter absolute right-0 top-[3.7rem] w-72 rounded-[1.5rem] border border-slate-200/80 bg-white/95 p-2 shadow-soft backdrop-blur-xl">
                <div className="signed-banner rounded-[1.1rem] px-4 py-4 text-white">
                  ...
                  <p className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-pine-950/15 px-2.5 py-1 text-xs font-bold text-pine-50"><Sparkles size={12} /> {t('shell.focus', 'Focus')}: {focus.shortLabel}</p>
                </div>
                <button type="button" onClick={() => onLocaleChange(locale === 'bn' ? 'en' : 'bn')} className="mt-2 flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-600 hover:bg-pine-50 hover:text-pine-800"><Languages size={16} /> {locale === 'bn' ? 'Switch to English' : 'বাংলায় দেখুন'}</button>
                <button type="button" onClick={() => { setAccountOpen(false); onOpenOnboarding(); }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-600 hover:bg-pine-50 hover:text-pine-800"><Settings2 size={16} /> {t('shell.personalize', 'Personalize MindHaven')}</button>
                <button type="button" onClick={() => navigate('profile')} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-600 hover:bg-pine-50 hover:text-pine-800"><UserRound size={16} /> {t('shell.profile', 'Privacy & profile')}</button>
                <button type="button" onClick={onLogout} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-500 hover:bg-red-50 hover:text-red-700"><LogOut size={16} /> {t('shell.signOut', 'Sign out')}</button>
              </div>
            )}
          </div>
      ...
      {mobileMenuOpen && (
        <div className="page-enter fixed inset-x-3 top-[5.4rem] z-40 max-h-[calc(100vh-7rem)] overflow-y-auto rounded-[1.75rem] border border-slate-200/80 bg-white/95 p-3 shadow-soft backdrop-blur-2xl lg:hidden">
          <div className="signed-banner mb-3 rounded-2xl p-4 text-white">
            <p className="truncate text-sm font-semibold">{user?.name}</p>
            <p className="mt-1 text-xs text-pine-50">{t('shell.focus', 'Your focus')}: {focus.shortLabel}</p>
          </div>
          <button type="button" onClick={() => navigate('checkin')} className="primary-button mb-2 w-full"><Plus size={17} /> {t('shell.checkIn', 'New daily check-in')}</button>
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <button key={item.id} type="button" onClick={() => navigate(item.id)} className={`flex items-center gap-2 rounded-xl px-3 py-2.5 text-left text-sm font-semibold ${activePage === item.id ? 'bg-pine-50 text-pine-900' : 'text-slate-500'}`}>
                  <Icon size={17} /> {labelFor(item)}
                </button>
              );
            })}
          </div>
          <button type="button" onClick={() => onLocaleChange(locale === 'bn' ? 'en' : 'bn')} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-pine-50 py-2.5 text-xs font-bold text-pine-700"><Languages size={15} /> {locale === 'bn' ? 'English' : 'বাংলা'}</button>
          <button type="button" onClick={() => { setMobileMenuOpen(false); onOpenOnboarding(); }} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-slate-50 py-2.5 text-xs font-bold text-slate-600"><Settings2 size={15} /> {t('shell.personalize', 'Personalize')}</button>
          <button type="button" onClick={onLogout} className="mt-2 flex w-full items-center justify-center gap-2 text-xs font-bold text-slate-500"><LogOut size={15} /> {t('shell.signOut', 'Sign out')}</button>
        </div>
      )}
      ...
      <nav className="fixed inset-x-3 bottom-3 z-30 flex items-center rounded-[1.6rem] border border-slate-200/80 bg-white/95 px-1.5 py-1.5 shadow-[0_14px_40px_rgba(26,47,43,0.14)] backdrop-blur-2xl lg:hidden" aria-label="Mobile wellbeing journey">
        {MOBILE_ITEMS.slice(0, 2).map((item) => <MobileNavButton key={item.id} item={item} label={labelFor(item, true)} active={activePage === item.id} onClick={() => navigate(item.id)} />)}
        ...
        {MOBILE_ITEMS.slice(2).map((item) => <MobileNavButton key={item.id} item={item} label={labelFor(item, true)} active={activePage === item.id} onClick={() => navigate(item.id)} />)}
      </nav>
    </div>
  );
}
