...
export const communityApi = {
  getPosts: (token, query = {}) =>
    apiRequest(`/community?${new URLSearchParams(query)}`, { token }),
  createPost: (token, body) => apiRequest('/community', { method: 'POST', token, body }),
  updatePost: (token, id, body) => apiRequest(`/community/${id}`, { method: 'PATCH', token, body }),
  deletePost: (token, id) => apiRequest(`/community/${id}`, { method: 'DELETE', token }),
  respond: (token, id, body) => apiRequest(`/community/${id}/response`, { method: 'POST', token, body: { body } }),
  updateResponse: (token, id, body) => apiRequest(`/community/${id}/response`, { method: 'PATCH', token, body: { body } }),
  report: (token, id, reason) => apiRequest(`/community/${id}/report`, { method: 'POST', token, body: { reason } }),
};
