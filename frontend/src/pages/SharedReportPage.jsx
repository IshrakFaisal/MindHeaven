      ...
      <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
        ...
        {payload && (() => {
          ...
          return <>
            ...
            <section className="card mt-6 p-6 sm:p-8"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="eyebrow">Report coverage</p><h2 className="mt-2 section-title">The records behind this summary</h2></div><span className="rounded-xl bg-pine-50 px-3 py-2 text-xs font-bold text-pine-700">Expires {formatDate(share.expiresAt)}</span></div><div className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-pine-50 p-4"><p className="text-2xl font-semibold">{report.moods.length}</p><p className="mt-1 text-xs font-bold text-slate-500">Mood entries</p></div><div className="rounded-2xl bg-pine-50 p-4"><p className="text-2xl font-semibold">{report.sleepLogs.length}</p><p className="mt-1 text-xs font-bold text-slate-500">Sleep logs</p></div><div className="rounded-2xl bg-pine-50 p-4"><p className="text-2xl font-semibold">{report.symptomLogs.length}</p><p className="mt-1 text-xs font-bold text-slate-500">Body-signal logs</p></div></div><p className="mt-6 rounded-2xl bg-amber-50 p-4 text-xs leading-6 text-amber-900">This page shows the current records inside the selected period. The owner can revoke this link at any time.</p></section>
          </>;
        })()}
      </main>
    </div>
