...
import { BookOpenText, CalendarDays, Check, CloudCog, Flame, Pencil, Search, Sparkles, Tag, Trash2, X } from 'lucide-react';
import { EmptyState, Notice, PageHeader, SummaryCard, Spinner } from '../components/Ui';
import MoodSlider from '../components/MoodSlider';
import { applyJournalTemplate, JOURNAL_TEMPLATES } from '../features/journal/journalTemplates';
import { trackerApi } from '../lib/api';
import { average, formatDate, localDateKey, moodColor, moodEmoji, moodLabel } from '../lib/format';

const initialForm = { moodValue: 7, moodType: 'numeric', title: '', note: '', tags: '' };
const draftKey = (userId) => `mindhaven-journal-draft-${userId || 'local'}`;
...
function MoodRecordMarker({ entry }) {
  const type = entry.moodType || 'numeric';
  if (type === 'color') {
    return <span className="mood-history-marker is-color" style={{ backgroundColor: moodColor(entry.moodValue) }}><span className="sr-only">Color scale: </span>{entry.moodValue}</span>;
  }
  if (type === 'emoji') {
    return <span className="mood-history-marker is-emoji" aria-label={`Emoji scale: ${moodLabel(entry.moodValue)}`}>{moodEmoji(entry.moodValue)}</span>;
  }
  return <span className="mood-history-marker is-numeric"><strong>{entry.moodValue}</strong><small>/10</small></span>;
}

export default function MoodPage({ token, data, onSaved, notify, userId }) {
  const storedDraft = readDraft(userId);
  const [form, setForm] = useState(() => ({ ...initialForm, ...storedDraft?.form }));
  const [activeTemplate, setActiveTemplate] = useState(storedDraft?.templateId || 'free');
  const [draftRestored] = useState(Boolean(storedDraft));
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => {
      const hasContent = form.title.trim() || form.note.trim() || form.tags.trim();
      if (hasContent) localStorage.setItem(draftKey(userId), JSON.stringify({ form, templateId: activeTemplate, updatedAt: new Date().toISOString() }));
      else localStorage.removeItem(draftKey(userId));
    }, 250);
    return () => window.clearTimeout(timeout);
  }, [activeTemplate, form, userId]);
        ...
        <form className="card p-5 sm:p-7" onSubmit={handleSubmit}>
          ...
          {!editingId && (
            <fieldset className="mt-6">
              <div className="flex items-center justify-between gap-3"><legend className="field-label flex items-center gap-2"><BookOpenText size={15} /> Reflection template</legend><span className="inline-flex items-center gap-1.5 text-xs font-semibold text-pine-600" role="status"><CloudCog size={13} /> {draftRestored ? 'Draft restored' : 'Autosaved locally'}</span></div>
              <p className="mt-1 text-xs leading-5 text-slate-400">Choose a prompt or keep a blank page. Selecting a template replaces the current title and note.</p>
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {JOURNAL_TEMPLATES.map((template) => (
                  <button key={template.id} type="button" onClick={() => { setActiveTemplate(template.id); setForm((current) => applyJournalTemplate(template.id, current)); }} aria-pressed={activeTemplate === template.id} className={`rounded-2xl border p-3 text-left transition ${activeTemplate === template.id ? 'border-pine-500 bg-pine-50' : 'border-slate-200 bg-white hover:border-pine-200'}`}>
                    <span className="block text-sm font-semibold text-pine-950">{template.label}</span>
          ...
          <fieldset className="mood-scale-style mt-6">
            ...
            <div className="mood-scale-style-options">
              {[
                ...
                <label
                  key={value}
                  className="mood-scale-style-option"
                >
                  <input
                    className="sr-only"
                    type="radio"
          ...
          <div className="mt-4" aria-live="polite">
            <MoodSlider mode={form.moodType} value={form.moodValue} onChange={(moodValue) => setForm((current) => ({ ...current, moodValue }))} disabled={submitting} />
          </div>
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="mt-0.5 text-xs font-medium text-slate-400">{formatDate(entry.createdAt, { year: true })} · {(entry.moodType || 'numeric')} scale</p>
                            </div>
