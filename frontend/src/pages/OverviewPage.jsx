...
const FOCUS_GUIDANCE = {
  ...
  stress: {
    ...
    detail: 'A quick anxiety and energy check can help you see when pressure tends to build.',
    page: 'symptoms',
...
export default function OverviewPage({ data, user, token, onSaved, notify, onNavigate, preferences }) {
  ...
  const activity = [
    ...
    ...data.symptoms.map((item) => ({ ...item, kind: 'Body signals', page: 'symptoms', Icon: Activity, summary: `Anxiety ${item.anxietyLevel || '—'} · Energy ${item.energyLevel || '—'}` })),
  ]
  ...
  const saveQuickMood = async () => {
    ...
    try {
      ...
      await Promise.all(
        selectedTags.map((label) => trackerApi.createTag(token, { moodEntry: mood._id, label })),
      );
      setNote('');
      setSelectedTags([]);
      await onSaved();
      setSaveState('saved');
      notify('Today’s check-in is saved.');
    } catch (error) {
      ...
      <header className="signed-page-hero overview-hero relative mb-6 overflow-hidden text-white">
        ...
        <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-center lg:gap-8">
          ...
          <div className="flex shrink-0 flex-col items-start gap-2 lg:items-center">
            ...
            <p className="text-xs text-pine-50">Mood, sleep, body signals & more</p>
          </div>
        </div>
      </header>
            ...
            <div className="mt-auto pt-5">
              <div className="flex flex-col justify-between gap-3 border-t border-pine-200/50 pt-4 sm:flex-row sm:items-center">
                <p className={`overview-save-status ${saveState === 'saved' ? 'is-saved' : ''}`} role="status">
                  ...
                  {submitting ? 'Saving your check-in…' : saveState === 'saved' ? 'Saved to your journal' : 'Not saved yet'}
                </p>
                <button type="submit" className="primary-button w-full min-w-32 sm:w-auto" disabled={submitting}>
                  {submitting ? <Spinner /> : <Check size={17} aria-hidden="true" />} {submitting ? 'Saving' : 'Save'}
                </button>
              </div>
            </div>
          </form>
