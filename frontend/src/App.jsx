...
export default function App() {
  ...
  useEffect(() => {
    document.documentElement.lang = preferences.locale === 'bn' ? 'bn' : 'en';
  }, [preferences.locale]);
  ...
  const authenticate = (response) => {
    const nextSession = {
      ...
      user: {
        ...
        therapistProfile: response.therapistProfile || null,
  ...
  const page = useMemo(() => {
    ...
    switch (activePage) {
      case 'checkin':
        return <CheckInPage {...sharedTrackerProps} data={data} onNavigate={setActivePage} preferences={preferences} userId={session.user._id} />;
      case 'mood':
        return <MoodPage {...sharedTrackerProps} data={data} userId={session.user._id} />;
      case 'symptoms':
      ...
      case 'care':
        return <CarePage token={session.token} notify={notify} locale={preferences.locale} />;
      case 'community':
        return <CommunityPage token={session.token} user={session.user} notify={notify} />;
      case 'profile':
        return (
          <ProfilePage
            token={session.token}
  ...
  return (
    <AppShell
      ...
      preferences={preferences}
      onOpenOnboarding={() => setOnboardingOpen(true)}
      onLocaleChange={(locale) => savePreferences({ ...preferences, locale })}
    >
      {loadError && (
        <div className="mb-5">
          <Notice type="error">
